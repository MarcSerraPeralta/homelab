...
for i in {1..12}; do
  ...
  # running the ifstat command takes 1 second, so no need for the 'sleep 1' instruction
  ...
  if (( i % 6 == 0 )); then
    ...
    for ((j=0; j<${#sums[@]}; j++)); do
      ...
      NET_USAGE=$(echo "${NET_USAGE}${avg},")
      sums[j]=0
    done
    NET_USAGE="${NET_USAGE::-1}" # remove trailing comma
