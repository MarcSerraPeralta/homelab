...
if [ ! -f "$LOGFILE_H" ]; then
  echo "timestamp,temperature_cpu,temperature_zone0,temperature_zone1,temperature_zone2" > "$LOGFILE_H"
fi
if [ ! -f "$LOGFILE_D" ]; then
  echo "timestamp,temperature_cpu,temperature_zone0,temperature_zone1,temperature_zone2" > "$LOGFILE_D"
fi
...
for i in {1..12}; do
  ...
  # instantaneous
  printf "timestamp,temperature_cpu,temperature_zone0,temperature_zone1,temperature_zone2\n$DATE,$TEMP_CPU,$TEMP_Z0,$TEMP_Z1,$TEMP_Z2" > "$LOGFILE_I"

  # last 1h
  echo "$DATE,$TEMP_CPU,$TEMP_Z0,$TEMP_Z1,$TEMP_Z2" >> "$LOGFILE_H"
  ...
  # last 24h
  AVE_TEMP_CPU=$(awk "BEGIN {printf $TEMP_CPU/6 + $AVE_TEMP_CPU}")
  ...
  if (( i % 6 == 0)); then
    AVE_TEMP_CPU=$(awk "BEGIN {printf \"%.1f\", $AVE_TEMP_CPU}")
    ...
    echo "$DATE,$AVE_TEMP_CPU,$AVE_TEMP_Z0,$AVE_TEMP_Z1,$AVE_TEMP_Z2" >> "$LOGFILE_D"
    # store only the last lines
